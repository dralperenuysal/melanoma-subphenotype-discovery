"""Melanoma-only UMAP+HDBSCAN clustering/validation for the PanDerm
re-embedding experiment. Same UMAP+HDBSCAN sweep / bootstrap / seed
pipeline as clustering/run_clustering_pipeline_melanoma_patchpool_cropped.py,
but on PanDerm CLS-equivalent embeddings (768-dim, one .npy per isic_id under
embeddings/panderm/{raw,cropped}/cls/) instead of DINOv2. Run twice -- once for
the raw cohort, once for the cropped cohort -- via a CLI arg selecting which.

Run with `.venv_cluster/bin/python` (default env segfaults on `import umap`).
Usage: python run_clustering_pipeline_melanoma_panderm.py {raw,cropped}
"""
import json
...
VARIANT = sys.argv[1]
assert VARIANT in ("raw", "cropped"), "usage: python run_clustering_pipeline_melanoma_panderm.py {raw,cropped}"
...
OUT_DIR = os.path.join(ROOT, "clustering", f"melanoma_only_panderm_{VARIANT}")
...
# Same grid as every other clustering run in this project -- kept identical
# for comparability, still systematic/documented rather than hand-picked.
...
if __name__ == "__main__":
    ...
    print(f"Loading PanDerm ({VARIANT}) CLS embeddings + lesion metadata...")
    isic_ids = sorted(f[:-4] for f in os.listdir(CLS_DIR) if f.endswith(".npy"))
    X = np.stack([np.load(os.path.join(CLS_DIR, f"{isic_id}.npy")) for isic_id in isic_ids]).astype(np.float32)
    ...
    report = dict(
        subset=f"melanoma_only_panderm_{VARIANT}", n_images=len(isic_ids),
        winning_config=dict(
