"""Melanoma-only Step 3+4 for ComBat-harmonized PanDerm embeddings (runs M/N).
Same pipeline as run_clustering_pipeline_melanoma_panderm.py, but loads the
pre-built ComBat-harmonized matrix (embeddings/panderm_{raw,cropped}_cls_combat.npy)
instead of per-image .npy files under embeddings/panderm/{variant}/cls/.

Run with `.venv_cluster/bin/python`.
Usage: python run_clustering_pipeline_melanoma_panderm_combat.py {raw,cropped}
"""
import json
...
VARIANT = sys.argv[1]
assert VARIANT in ("raw", "cropped"), "usage: python run_clustering_pipeline_melanoma_panderm_combat.py {raw,cropped}"

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
COMBAT_NPY = os.path.join(ROOT, "embeddings", f"panderm_{VARIANT}_cls_combat.npy")
COMBAT_IDS_CSV = os.path.join(ROOT, "embeddings", f"panderm_{VARIANT}_cls_combat_isic_ids.csv")
METADATA_CSV = os.path.join(ROOT, "data", "processed", "quality_filtered_metadata.csv")

OUT_DIR = os.path.join(ROOT, "clustering", f"melanoma_only_panderm_{VARIANT}_combat")
...
if __name__ == "__main__":
    ...
    print(f"Loading ComBat-harmonized PanDerm ({VARIANT}) CLS embeddings + lesion metadata...")
    isic_ids = pd.read_csv(COMBAT_IDS_CSV)["isic_id"].tolist()
    X = np.load(COMBAT_NPY).astype(np.float32)
    ...
    report = dict(
        subset=f"melanoma_only_panderm_{VARIANT}_combat", n_images=len(isic_ids),
        winning_config=dict(
