"""Melanoma-only variant of the UMAP+HDBSCAN clustering/validation pipeline.
Same pipeline as run_clustering_pipeline.py, but
restricted to histopathologically confirmed melanoma images only
(diagnosis_3 in {"Melanoma, NOS", "Melanoma in situ", "Melanoma Invasive"}),
~6,714 images, to find subphenotypes within melanoma rather than rediscovering
the malignant/benign split. Outputs go to clustering/melanoma_only/ so the
mixed-cohort results are kept untouched as a secondary comparison.
...
OUT_DIR = os.path.join(ROOT, "clustering", "melanoma_only")
...
# Same grid as the mixed-cohort run — kept identical for comparability, still
# systematic/documented rather than hand-picked.
...
if __name__ == "__main__":
    ...
    report = dict(
        subset="melanoma_only", n_images=len(isic_ids),
        winning_config=dict(
