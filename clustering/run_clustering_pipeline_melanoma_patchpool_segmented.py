...
(embeddings/melanoma_patch_meanpool_segmented.npy, built by
...
POOLED_NPY = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_segmented.npy")
POOLED_IDS_CSV = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_segmented_isic_ids.csv")
METADATA_CSV = os.path.join(ROOT, "data", "processed", "quality_filtered_metadata.csv")

OUT_DIR = os.path.join(ROOT, "clustering", "melanoma_only_patchpool_segmented")
...
if __name__ == "__main__":
    ...
    report = dict(
        subset="melanoma_only_patchpool_segmented", n_images=len(isic_ids),
        winning_config=dict(
