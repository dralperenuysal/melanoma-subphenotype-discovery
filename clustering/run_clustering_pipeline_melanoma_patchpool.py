...
(embeddings/melanoma_patch_meanpool.npy, built by
...
POOLED_NPY = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool.npy")
POOLED_IDS_CSV = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_isic_ids.csv")
METADATA_CSV = os.path.join(ROOT, "data", "processed", "quality_filtered_metadata.csv")

OUT_DIR = os.path.join(ROOT, "clustering", "melanoma_only_patchpool")
...
if __name__ == "__main__":
    ...
    report = dict(
        subset="melanoma_only_patchpool", n_images=len(isic_ids),
        winning_config=dict(
