"""Melanoma-only UMAP+HDBSCAN clustering/validation, ComBat-harmonized
patch-meanpool variant (batch-effect harmonization sensitivity analysis).
Identical pipeline to
run_clustering_pipeline_melanoma_patchpool_cropped.py but clusters on
ComBat-harmonized embeddings (embeddings/melanoma_patch_meanpool_cropped_combat.npy,
built by embeddings/build_melanoma_combat.py, batch = attribution/source
institution) instead of the raw cropped patch-meanpool embeddings, to test
whether explicit batch-effect correction removes the acquisition-source
confound found in the cropped-only run (cluster 0 was 81% Hospital Clinic de
Barcelona) while preserving any real melanoma-progression clinical signal.

Outputs go to clustering/melanoma_only_patchpool_combat/ — prior outputs
(melanoma_only_patchpool/, melanoma_only_patchpool_cropped/) are untouched.
...
POOLED_NPY = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_cropped_combat.npy")
POOLED_IDS_CSV = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_cropped_combat_isic_ids.csv")
METADATA_CSV = os.path.join(ROOT, "data", "processed", "quality_filtered_metadata.csv")

OUT_DIR = os.path.join(ROOT, "clustering", "melanoma_only_patchpool_combat")
...
if __name__ == "__main__":
    ...
    report = dict(
        subset="melanoma_only_patchpool_combat", n_images=len(isic_ids),
        winning_config=dict(
