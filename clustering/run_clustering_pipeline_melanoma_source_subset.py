"""Melanoma-only Step 3+4, patch-meanpool variant, restricted to a single
acquisition-source (`attribution`) subgroup — a sensitivity analysis to check
whether within-melanoma cluster structure survives once the cross-institution
device-style confound is controlled for by construction.

Parameterized over run_clustering_pipeline_melanoma_patchpool_cropped.py so we
don't duplicate the pipeline per source. Usage:
    .venv_cluster/bin/python clustering/run_clustering_pipeline_melanoma_source_subset.py \
        --tag barcelona --attribution "Hospital Clínic de Barcelona" \
        --min-cluster-sizes 10 15 25 40
"""
import argparse
import json
import multiprocessing as mp
import os
import time

import numpy as np
import pandas as pd
from sklearn.metrics import adjusted_rand_score, davies_bouldin_score, silhouette_score
from umap.umap_ import UMAP
import hdbscan

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
POOLED_NPY = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_cropped.npy")
POOLED_IDS_CSV = os.path.join(ROOT, "embeddings", "melanoma_patch_meanpool_cropped_isic_ids.csv")
METADATA_CSV = os.path.join(ROOT, "data", "processed", "quality_filtered_metadata.csv")

UMAP_GRID = [(n, d) for n in (15, 30, 50) for d in (0.0, 0.1, 0.25)]
MAIN_SEED = 42
BOOTSTRAP_N_ITER = 100
BOOTSTRAP_SUBSAMPLE_FRAC = 0.8
BOOTSTRAP_SEED_BASE = 1000
SEED_SENSITIVITY_SEEDS = [1, 2, 3, 4, 5]
ARI_STABILITY_THRESHOLD = 0.6


def fit_umap(X, n_neighbors, min_dist, seed):
    return UMAP(n_neighbors=n_neighbors, min_dist=min_dist, n_components=2,
                random_state=seed).fit_transform(X)


def fit_hdbscan(embedding2d, min_cluster_size, min_samples):
    return hdbscan.HDBSCAN(min_cluster_size=min_cluster_size,
                            min_samples=min_samples).fit_predict(embedding2d)


def cluster_metrics(embedding2d, labels):
    n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
    noise_frac = float(np.mean(labels == -1))
    mask = labels != -1
    if n_clusters < 2 or mask.sum() < 2:
        return n_clusters, noise_frac, float("nan"), float("nan")
    return (n_clusters, noise_frac,
            silhouette_score(embedding2d[mask], labels[mask]),
            davies_bouldin_score(embedding2d[mask], labels[mask]))


def _sweep_worker(args):
    n_neighbors, min_dist = args
    emb = fit_umap(X, n_neighbors, min_dist, seed=MAIN_SEED)
    rows = []
    for min_cluster_size, min_samples in HDBSCAN_GRID:
        labels = fit_hdbscan(emb, min_cluster_size, min_samples)
        n_clusters, noise_frac, sil, db = cluster_metrics(emb, labels)
        rows.append(dict(n_neighbors=n_neighbors, min_dist=min_dist,
                          min_cluster_size=min_cluster_size, min_samples=min_samples,
                          n_clusters=n_clusters, noise_frac=noise_frac,
                          silhouette=sil, davies_bouldin=db))
    return (n_neighbors, min_dist), emb, rows


def choose_best_config(sweep_df):
    candidates = sweep_df[
        (sweep_df["n_clusters"] >= 2) & (sweep_df["n_clusters"] <= 15)
        & (sweep_df["noise_frac"] < 0.3) & sweep_df["silhouette"].notna()
    ]
    fallback_used = False
    if candidates.empty:
        fallback_used = True
        candidates = sweep_df[(sweep_df["n_clusters"] >= 2) & sweep_df["silhouette"].notna()]
    return candidates.loc[candidates["silhouette"].idxmax()], fallback_used


def _bootstrap_worker(i):
    rng = np.random.default_rng(BOOTSTRAP_SEED_BASE + i)
    n_sample = int(round(BOOTSTRAP_SUBSAMPLE_FRAC * len(unique_lesions)))
    subsample_lesions = rng.choice(unique_lesions, size=n_sample, replace=False)
    mask = np.isin(lesion_ids, subsample_lesions)
    emb_sub = fit_umap(X[mask], best_n_neighbors, best_min_dist, seed=BOOTSTRAP_SEED_BASE + i)
    labels_sub = fit_hdbscan(emb_sub, best_min_cluster_size, best_min_samples)
    return adjusted_rand_score(full_labels[mask], labels_sub)


def _seed_worker(seed):
    emb = fit_umap(X, best_n_neighbors, best_min_dist, seed=seed)
    labels = fit_hdbscan(emb, best_min_cluster_size, best_min_samples)
    return seed, adjusted_rand_score(full_labels, labels)


if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--tag", required=True)
    ap.add_argument("--attribution", required=True)
    ap.add_argument("--min-cluster-sizes", nargs="+", type=int, default=[10, 15, 25, 40])
    ap.add_argument("--min-samples", nargs="+", type=int, default=[5, 15])
    args = ap.parse_args()

    HDBSCAN_GRID = [(mcs, ms) for mcs in args.min_cluster_sizes for ms in args.min_samples]
    N_WORKERS_SWEEP = min(len(UMAP_GRID), max(1, mp.cpu_count() - 2))
    N_WORKERS_BOOTSTRAP = min(16, max(1, mp.cpu_count() - 2))

    OUT_DIR = os.path.join(ROOT, "clustering", f"melanoma_only_{args.tag}_cropped")
    os.makedirs(OUT_DIR, exist_ok=True)

    t_start = time.time()
    all_ids = pd.read_csv(POOLED_IDS_CSV)["isic_id"].tolist()
    X_all = np.load(POOLED_NPY).astype(np.float32)
    meta = pd.read_csv(METADATA_CSV, low_memory=False).set_index("isic_id")

    subset_mask = meta.loc[all_ids, "attribution"] == args.attribution
    isic_ids = [i for i, m in zip(all_ids, subset_mask.to_numpy()) if m]
    X = X_all[subset_mask.to_numpy()]
    print(f"[{args.tag}] subset n={len(isic_ids)} (attribution == {args.attribution!r})")
    assert X.shape[0] == len(isic_ids) and len(isic_ids) > 100

    lesion_ids = meta.loc[isic_ids, "lesion_id"].fillna(pd.Series(isic_ids, index=isic_ids)).to_numpy()
    unique_lesions = np.unique(lesion_ids)

    print(f"[{args.tag}] Sweep: {len(UMAP_GRID)} UMAP fits x {len(HDBSCAN_GRID)} HDBSCAN configs...")
    with mp.Pool(N_WORKERS_SWEEP) as pool:
        sweep_results = pool.map(_sweep_worker, UMAP_GRID)
    embeddings_by_config = {key: emb for key, emb, _ in sweep_results}
    sweep_df = pd.DataFrame([row for _, _, rows in sweep_results for row in rows])
    sweep_df.to_csv(os.path.join(OUT_DIR, "sweep_results.csv"), index=False)

    best, fallback_used = choose_best_config(sweep_df)
    best_n_neighbors, best_min_dist = int(best["n_neighbors"]), float(best["min_dist"])
    best_min_cluster_size, best_min_samples = int(best["min_cluster_size"]), int(best["min_samples"])
    print(f"[{args.tag}] Winning config (fallback_used={fallback_used}): "
          f"n_neighbors={best_n_neighbors}, min_dist={best_min_dist}, "
          f"min_cluster_size={best_min_cluster_size}, min_samples={best_min_samples}, "
          f"silhouette={best['silhouette']:.4f}, n_clusters={int(best['n_clusters'])}, "
          f"noise_frac={best['noise_frac']:.3f}")

    final_emb = embeddings_by_config[(best_n_neighbors, best_min_dist)]
    full_labels = fit_hdbscan(final_emb, best_min_cluster_size, best_min_samples)
    pd.DataFrame({"isic_id": isic_ids, "cluster_label": full_labels}).to_csv(
        os.path.join(OUT_DIR, "final_cluster_labels.csv"), index=False)
    pd.DataFrame({"isic_id": isic_ids, "umap_x": final_emb[:, 0], "umap_y": final_emb[:, 1]}).to_csv(
        os.path.join(OUT_DIR, "final_umap_2d.csv"), index=False)

    print(f"[{args.tag}] Bootstrap: {BOOTSTRAP_N_ITER} iters, lesion-level 80% subsample...")
    with mp.Pool(N_WORKERS_BOOTSTRAP) as pool:
        bootstrap_aris = np.array(pool.map(_bootstrap_worker, range(BOOTSTRAP_N_ITER)))
    print(f"[{args.tag}] Bootstrap mean ARI={bootstrap_aris.mean():.4f}, std={bootstrap_aris.std():.4f}")

    with mp.Pool(min(len(SEED_SENSITIVITY_SEEDS), N_WORKERS_SWEEP)) as pool:
        seed_results = pool.map(_seed_worker, SEED_SENSITIVITY_SEEDS)

    is_stable = bool(bootstrap_aris.mean() >= ARI_STABILITY_THRESHOLD)
    report = dict(
        subset=f"melanoma_only_{args.tag}_cropped", attribution=args.attribution,
        n_images=len(isic_ids),
        winning_config=dict(n_neighbors=best_n_neighbors, min_dist=best_min_dist,
                             min_cluster_size=best_min_cluster_size, min_samples=best_min_samples,
                             fallback_used=fallback_used),
        main_seed=MAIN_SEED, n_clusters=int(best["n_clusters"]), noise_frac=float(best["noise_frac"]),
        silhouette=float(best["silhouette"]), davies_bouldin=float(best["davies_bouldin"]),
        bootstrap=dict(n_iter=BOOTSTRAP_N_ITER, subsample_frac=BOOTSTRAP_SUBSAMPLE_FRAC,
                        level="lesion", seed_base=BOOTSTRAP_SEED_BASE,
                        mean_ari=float(bootstrap_aris.mean()), std_ari=float(bootstrap_aris.std()),
                        min_ari=float(bootstrap_aris.min()), max_ari=float(bootstrap_aris.max()),
                        ari_values=bootstrap_aris.tolist(),
                        stability_threshold=ARI_STABILITY_THRESHOLD, is_stable=is_stable),
        seed_sensitivity=dict(reference_seed=MAIN_SEED,
                               aris={str(s): a for s, a in seed_results}),
        wall_clock_seconds=time.time() - t_start,
    )
    with open(os.path.join(OUT_DIR, "step4_validation_report.json"), "w") as f:
        json.dump(report, f, indent=2)

    print(f"[{args.tag}] " + ("STABLE" if is_stable else "UNSTABLE") +
          f" (mean ARI={bootstrap_aris.mean():.4f}, threshold={ARI_STABILITY_THRESHOLD})")
    print(f"[{args.tag}] Total wall clock: {time.time()-t_start:.1f}s")

    assert len(full_labels) == len(isic_ids) == X.shape[0]
    assert len(bootstrap_aris) == BOOTSTRAP_N_ITER
    print(f"[{args.tag}] Sanity checks passed.")
