"""UMAP+HDBSCAN hyperparameter sweep and internal validation on the DINOv2 CLS
embeddings (mixed-diagnosis cohort). Run with `.venv_cluster/bin/python`
(the project's default env has a broken tensorflow+torch+numpy combo that
segfaults on `import umap` — confirmed while building this script).

Reproducibility policy (mandatory): every UMAP/HDBSCAN fit below
takes an explicit fixed random_state/seed passed at the call site — nothing
relies on a library default or hidden global random state.

Bootstrap stability subsamples at the LESION level, not the image level --
otherwise near-duplicate images from the
same lesion would inflate the Adjusted Rand Index artificially.
"""
import json
...
SWEEP_CSV = os.path.join(ROOT, "clustering", "cluster_assignments", "sweep_results.csv")
FINAL_LABELS_CSV = os.path.join(ROOT, "clustering", "cluster_assignments", "final_cluster_labels.csv")
FINAL_UMAP_CSV = os.path.join(ROOT, "clustering", "umap_projections", "final_umap_2d.csv")
VALIDATION_JSON = os.path.join(ROOT, "clustering", "stability_analysis", "step4_validation_report.json")

# Sweep grid — modest and systematic, not exhaustive (ponytail: a small grid
# already satisfies the "systematic, not hand-picked" requirement).
...
ARI_STABILITY_THRESHOLD = 0.6  # pre-registered acceptance threshold
...
def cluster_metrics(embedding2d, labels):
    """Silhouette/DB index computed on non-noise points only (standard practice
    for HDBSCAN evaluation) — NaN when fewer than 2 clusters remain."""
...
def choose_best_config(sweep_df):
    """Selection rule (documented per the no-cherry-picking principle):
    highest silhouette among configs with 2-15 clusters and <30% noise. Falls
    back to highest silhouette among any config with >=2 clusters if nothing
    meets the noise/cluster-count band."""
    candidates = sweep_df[
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.dirname(FINAL_UMAP_CSV), exist_ok=True)
    os.makedirs(os.path.dirname(VALIDATION_JSON), exist_ok=True)

    print("Loading metadata + CLS embeddings...")
    df = pd.read_csv(METADATA_CSV, low_memory=False)
    df["lesion_id"] = df["lesion_id"].fillna(df["isic_id"])  # missing lesion_id -> singleton lesion
    ...
    # Final full-data fit reuses the sweep's UMAP embedding for the winning (n_neighbors, min_dist).
    ...
    assert all(a <= 1.0 for a in bootstrap_aris)  # ARI <= 1.0 always; can go negative (worse than random)
